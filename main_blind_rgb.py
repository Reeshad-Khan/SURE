...
parser.add_argument('--sigma', dest='sigma', type=float, default=0.01, help='noise level (for evaluation and testing)')
...
parser.add_argument('--gpu', dest='gpu', default='3', help='which gpu to use')
...
def denoiser_train(denoiser, lr):
    args.data = args.data+str(args.sigma)
    eval_files = glob('./Dataset/testset/{}/*.png'.format(args.eval_set))
    denoiser.train(args.data, eval_files, batch_size=args.batch_size, epoch=args.epoch, lr=lr, sigma=args.sigma)


def denoiser_test(denoiser, save_dir):

    if not os.path.exists('./Dataset/testset/{}/sigma'+args.ckpt_dir):
        os.makedirs('./Dataset/testset/{}/sigma'+args.ckpt_dir)
        noisy_files = natsort.natsorted(glob('./Dataset/testset/{}/sigma'+args.ckpt_dir+'/*.npy'.format(args.test_set)))
...
def main(_):
    ...
    name = "CDnCNN_" + args.cost + '_' + str(args.sigma)
    ...
    if args.use_gpu:
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            model = denoiser(sess, sigma=args.sigma, eps=eps, cost_str=args.cost, ckpt_dir=ckpt_dir, sample_dir=sample_dir, log_dir=log_dir)
            if args.phase == 'train':
