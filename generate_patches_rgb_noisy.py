...
parser.add_argument('--sigma', dest='sigma', type=float, default=0.01, help='sigma value')
...
gt = np.load(os.path.join(args.gt_dir, 'gt_rgb_clean_patches.npy'), mmap_mode='r')
...
gt_noisy        = np.array(gt).astype(np.float32) + np.array([np.random.normal(0, args.sigma, (50,50,1))], dtype='float32')  
gt_path_data    = os.path.join(args.save_dir, 'gt_rgb_noisy_' + str(args.sigma) + '_patches.npy')
...
sigmavec     = np.random.uniform(args.gt_max + 0.1, args.ny_max, (patches, )).astype(np.float32)
sigma_dif    = np.sqrt(np.square(sigmavec) - np.square(sigma_imperfect))
noisy        = gt_noisy + np.array([np.random.normal(0, args.sigma, (50,50,1))], dtype='float32')  
ny_datapath  = os.path.join(args.save_dir, 'gt_rgb_noisy_' + str(args.sigma) + '_patches.npy')
